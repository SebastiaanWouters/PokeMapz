...
if ($d["lastpokemon"] == "true") {
    if ($lastpokemon != 'true') {
        ...
        if (isset($_GET['timestamp'])) {
            ...
            $timestamp = $timestamp - 25200; //21600 = 6 hours (Increase to 21800 if missing pokemon to allow for slight time difference)
...
function get_gyms($swLat, $swLng, $neLat, $neLng, $tstamp = 0, $oSwLat = 0, $oSwLng = 0, $oNeLat = 0, $oNeLng = 0) {
    ...
    foreach ($datas as $row) {
        ...
        $lm = $lm + 25200; //21600 = 6 hours
        $ls = strtotime($row["last_updated"]);
        $ls = $ls * 1000;
        $ls = $ls + 25200; //21600 = 6 hours
