...
function gymLabel(teamName, teamId, gymPoints, latitude, longitude, lastScanned = null, name = null, members = [], gymId) {
    ...
    if (lastScanned) {
        ...
        lastScannedStr = `${lastScannedDate.getFullYear()}-${pad(lastScannedDate.getMonth() + 1)}-${pad(lastScannedDate.getDate())} ${pad(lastScannedDate.getHours()+7)}:${pad(lastScannedDate.getMinutes())}:${pad(lastScannedDate.getSeconds())}`
    } else {
...
function processGyms(i, item) {
    ...
    if (Store.get('showLastUpdatedGymsOnly')) {
        ...
        var t = now.getTime()
        if ((Store.get('showLastUpdatedGymsOnly') * 3600 * 1000) + item.last_scanned < t) {
            removeGymFromMap(item['gym_id'])
            return true
        }
    }
